...
import adaptiveEngine from "../engine/adaptiveLearningEngine"; // Add this import
...
function CourseDetail() {
  ...
  const [showRemedial, setShowRemedial] = useState(false); // Add this state
  ...
  // Separate useEffect for adaptive learning
  useEffect(() => {
    if (course && currentUser) {
      // Check if student is struggling with this topic
      const performance = adaptiveEngine.analyzePerformance(currentUser);
      const strugglingWithTopic = performance.weaknesses.some((w) =>
        course.title.toLowerCase().includes(w.topic.toLowerCase()),
      );

      if (strugglingWithTopic) {
        // Show remedial content or simplified explanations
        setShowRemedial(true);
      }

      // Adjust quiz passing score based on performance
      if (course.modules) {
        const adjustedModules = course.modules.map((module) => {
          if (module.quiz) {
            const topicPerformance = performance.weaknesses.find((w) =>
              module.title.toLowerCase().includes(w.topic.toLowerCase()),
            );

            if (topicPerformance) {
              // Lower passing score for struggling students
              module.quiz.passingScore = Math.max(
                50,
                (module.quiz.passingScore || 70) - 10,
              );
            } else {
              // Increase difficulty for excelling students
              module.quiz.passingScore = Math.min(
                90,
                (module.quiz.passingScore || 70) + 5,
              );
            }
          }
          return module;
        ...
        setCourse({ ...course, modules: adjustedModules });
      }
    }
  }, [course, currentUser]);
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <PrerequisiteChecker course={course}>
        <main className="flex-1 p-8">
          {/* Course Header */}
          <div className="bg-white rounded-xl shadow-lg p-6 mb-6">
            ...
            {/* Remedial Content Notice */}
            {showRemedial && (
              <div className="mt-4 p-4 bg-yellow-100 border-l-4 border-yellow-500 rounded">
                <p className="text-yellow-800">
                  <span className="font-bold">📚 Recommended Review:</span>{" "}
                  Based on your performance, we recommend taking extra time with
                  this module. Don't worry - we're here to help!
                </p>
              </div>
            )}
